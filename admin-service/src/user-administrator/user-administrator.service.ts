...
export class UserAdministratorService {
    ...
    async create(userAdministrator: CreateUserAdministratorDto): Promise<UserAdministrator> {
        const newUserAdministrator = this.userAdministratorRepository.create({
            userId: userAdministrator.userId,
        });
        await this.userAdministratorRepository.persistAndFlush(newUserAdministrator);
        return newUserAdministrator;
    }

    async update(id: string, userAdministratorInfo: CreateUserAdministratorDto): Promise<UserAdministrator> {
        const userAdministrator = await this.findOne(id);
        if (!userAdministrator) throw new NotFoundException('User Administrator data not found');
    ...
    async delete(id: string): Promise<void> {
        ...
        return await this.userAdministratorRepository.removeAndFlush(userAdministrator);
    }
}
